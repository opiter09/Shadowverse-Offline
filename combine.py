...
def createImages(table):
    ...
    for card in table:
        ...
        text = [ "", "", card["card_name"] + "   " + str(card["cost"]), "[" + rarities[card["rarity"]] + "] " + types[card["char_type"]],
        classes[card["clan"]] + " " + card["tribe_name"], "", "" ] \
        ...
        for line in text:
            if (len(line) != 0):
                new = textwrap.wrap(line, width = 38)
                for item in new:
                    if ((len(item) % 38) != 0):
                        temp = temp + "  " + item + (" " * (38 - (len(item) % 38))) + "  "
                    else:
                        temp = temp + "  " + item + "  "
            else:
                temp = temp + line + (" " * 42)

        im = Image.new("RGBA", size = (336, 464), color = (255, 255, 255)) # 42 x 58
        count = -1
        for ch in temp:
            count = count + 1
            if (count >= (42 * 58)):
                ...
                im.paste(sub, ((count % 42) * 8, math.floor(count / 42) * 8))
                sub.close()
        ...
        text = [ "", "", card["card_name"] + "   " + str(card["cost"]), "EVOLVED", "[" + rarities[card["rarity"]] + "] " + types[card["char_type"]],
        classes[card["clan"]] + " " + card["tribe_name"], "", "" ] \
